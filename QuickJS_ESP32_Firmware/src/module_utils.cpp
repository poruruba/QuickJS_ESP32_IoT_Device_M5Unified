...
static void formB(uint8_t b, char *p_hex){
  uint8_t t = (b >> 4) & 0x0f;
  if( 0 <= t && t <= 9 )
    p_hex[0] = '0' + t;
  else if( 0x0a <= t && t <= 0x0f)
    p_hex[0] = 'a' + (t - 0x0a);
...
static JSValue utils_hex2array(JSContext *ctx, JSValueConst jsThis,
                                     int argc, JSValueConst *argv)
{
  const char* p_hex = JS_ToCString(ctx, argv[0]);
  if( p_hex == NULL )
    return JS_EXCEPTION;
  uint32_t len = strlen(p_hex);
  if( (len % 2) != 0){
    JS_FreeCString(ctx, p_hex);
    return JS_EXCEPTION;
  }

  uint8_t *p_array = (uint8_t*)malloc(len / 2);
  if( p_array == NULL ){
    JS_FreeCString(ctx, p_hex);
    return JS_EXCEPTION;
  }
  for( int i = 0 ; i < len ; i += 2 ){
    uint8_t val = toC(p_hex[i]) << 8;
    val |= toC(p_hex[i + 1]);
    p_array[i / 2] = val;
  }
...
static JSValue utils_array2hex(JSContext *ctx, JSValueConst jsThis,
                                     int argc, JSValueConst *argv)
{
  uint8_t *p_buffer;
  uint8_t unit_size;
  ...
  char *p_hex = (char*)malloc(unit_num * 2 + 1);
  if( p_hex == NULL ){
    JS_FreeValue(ctx, vbuffer);
    return JS_EXCEPTION;
  }
  for( int i = 0 ; i < unit_num; i++ ){
    formB(p_buffer[i], &p_hex[i * 2]);
  }
  p_hex[unit_num * 2] = '\0';
  JS_FreeValue(ctx, vbuffer);

  JSValue value = JS_NewString(ctx, p_hex);
  free(p_hex);
...
static const JSCFunctionListEntry utils_funcs[] = {
    ...
    JSCFunctionListEntry{"hex2array", 0, JS_DEF_CFUNC, 0, {
                          func : {1, JS_CFUNC_generic, utils_hex2array}
                        }},
    JSCFunctionListEntry{"array2hex", 0, JS_DEF_CFUNC, 0, {
                          func : {1, JS_CFUNC_generic, utils_array2hex}
                        }},
