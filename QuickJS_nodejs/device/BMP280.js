...
class BMP280{
  ...
  async begin(addr = BMP280_ADDRESS, chipid = BMP280_CHIPID){
    ...
    var ret = await this.read8(BMP280_REGISTER_CHIPID);
    if( ret != chipid )
      throw 'chipid mismatch';

    await this.readCoefficients();
    await this.setSampling();
    await this.setSampling(MODE_NORMAL, SAMPLING_X2, SAMPLING_X16, FILTER_X16, STANDBY_MS_500);
  ...
  async readPressure() {
    ...
    var adc_P = await this.read24(BMP280_REGISTER_PRESSUREDATA);
    adc_P >>= 4;
  ...
  async readTemperature() {
   var adc_T = await this.read24(BMP280_REGISTER_TEMPDATA);
   adc_T >>= 4;
  ...
  async readCoefficients(){
    this._bmp280_calib.dig_T1 = await this.read16_LE(BMP280_REGISTER_DIG_T1);
    this._bmp280_calib.dig_T2 = await this.readS16_LE(BMP280_REGISTER_DIG_T2);
    this._bmp280_calib.dig_T3 = await this.readS16_LE(BMP280_REGISTER_DIG_T3);
  
    this._bmp280_calib.dig_P1 = await this.read16_LE(BMP280_REGISTER_DIG_P1);
    this._bmp280_calib.dig_P2 = await this.readS16_LE(BMP280_REGISTER_DIG_P2);
    this._bmp280_calib.dig_P3 = await this.readS16_LE(BMP280_REGISTER_DIG_P3);
    this._bmp280_calib.dig_P4 = await this.readS16_LE(BMP280_REGISTER_DIG_P4);
    this._bmp280_calib.dig_P5 = await this.readS16_LE(BMP280_REGISTER_DIG_P5);
    this._bmp280_calib.dig_P6 = await this.readS16_LE(BMP280_REGISTER_DIG_P6);
    this._bmp280_calib.dig_P7 = await this.readS16_LE(BMP280_REGISTER_DIG_P7);
    this._bmp280_calib.dig_P8 = await this.readS16_LE(BMP280_REGISTER_DIG_P8);
    this._bmp280_calib.dig_P9 = await this.readS16_LE(BMP280_REGISTER_DIG_P9);
  }

  async setSampling(mode = MODE_NORMAL, tempSampling = SAMPLING_X16, pressSampling = SAMPLING_X16, filter = FILTER_OFF, duration = STANDBY_MS_1){
    ...
    await this.write8(BMP280_REGISTER_CONFIG, this._configReg_get());
    await this.write8(BMP280_REGISTER_CONTROL, this._measReg_get());
  }
  ...
  async write8(reg, value) {
    await this.wire.beginTransmission(this.address);
    var ret = await this.wire.write([reg, value]);
    if( ret != 2 )
      throw 'failed';
  ...
  async read16_LE(reg) {
    var temp = await this.read16(reg);
    return (temp >> 8) | ((temp & 0xff) << 8);
  }
  
  async readS16_LE(reg) {
    var ret = await this.read16_LE(reg);
    if( ret & 0x8000 )
      return ~ret + 1;
    else
      return ret;
  ...
  async read8(reg) {
    await this.wire.beginTransmission(this.address);
    var ret = await this.wire.write(reg);
    if( ret != 1 )
      throw 'failed';
    var ret = await this.wire.endTransmission();
    if( ret != 0 )
  ...
  async read16(reg) {
    ...
    if( ret != 0 )
      throw 'failed';
    var ret = await this.wire.requestFrom(this.address, 2);
    if( ret != 2 )
      throw 'failed';
    var ret = await this.wire.read(2);
  ...
  async read24(reg) {
    ...
    if( ret != 0 )
      throw 'failed';
    var ret = await this.wire.requestFrom(this.address, 3);
    if( ret != 3 )
      throw 'failed';
    var ret = await this.wire.read(3);
